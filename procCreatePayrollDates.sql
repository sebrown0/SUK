CREATE DEFINER=`root`@`localhost` PROCEDURE `create_payroll_dates`(IN tax_year varchar(4))
BEGIN	
	DECLARE payNum INT;
    ...
    WHILE payNum <= 12 DO		
		INSERT INTO 
			`payroll_dates` (`tax_year`, `pay_frequency_id`, `payroll_number`, `date_from`, `date_to`, `run_payroll_on`, `pay_emps_on`) 
		VALUES 
