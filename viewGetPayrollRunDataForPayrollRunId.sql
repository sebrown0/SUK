...
VIEW `payroll_data_for_payroll_run_id` AS
    SELECT 
    ...
    FROM
        ...
        JOIN `all_emp_basic_for_payroll_run_id` `basic` ON ((`basic`.`employee_payroll_id` = `gross`.`employee_payroll_id`)))
    ...
    WHERE
        (`gross`.`payroll_run_id` = GET_PAYROLL_RUN_ID_PARAM())